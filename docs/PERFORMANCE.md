...
| Held voice bounded | Every species, `E3` held six seconds in played mode, `mold` moved to 0.7 half way; peak per second on the master bus tap | finite, under 1.5, last second under four times the first | yes |
...
10. **Mold's held voice was unstable since 1.0 and nothing measured it.** The 1.3 lab's first blind held note on Mold went silent after three seconds: the master bus was NaN. Its comb resonance and feedback delay LFOs were connected on top of the ramped levels, and a connected LFO adds to a param, so the loop gain passed 1. Every earlier row either used short generative notes, held a note for under three seconds, or measured a feature and found "none", which is what NaN looks like to a comparison. The 1.2 audibility result for Mold (finding 8) was therefore measuring a blown up graph, not the controls. Fixed by running those LFOs bipolar around a capped level; Bacteria's room size LFO had the same wiring. The new `held voice stays bounded` row blocks on it. After the fix a held Mold note peaks at 0.014 against Seed's 0.34, a level question for the sound pass, and a held Bacteria note is one 16 ms particle followed by silence, a design question for the verdict.
