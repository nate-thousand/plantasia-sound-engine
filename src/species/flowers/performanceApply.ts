...
export function applyFlowersPerformance(
  ...
  const attack = FLOWERS_ATTACK * Math.max(0.05, targets.attackMult);
  if (!gate || gate.changed('pwmWidth', pwmWidth, 0.005) || gate.changed('attack', attack, 0.002)) {
    synth.pwmPoly.set({
  ...
  if (!gate || gate.changed('spread', spread, 0.05)) {
    synth.sawPoly.set({
